...
export const Route = createFileRoute("/services/$slug")({
  ...
  notFoundComponent: () => (
    <div className="container-page py-32 text-center">
      ...
      <Link to="/services" className="mt-6 inline-flex underline">Back to services</Link>
    </div>
  ),
    ...
    <>
      <section className="relative overflow-hidden border-b border-border">
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(110deg, oklch(0.13 0.03 265 / 0.92), oklch(0.13 0.03 265 / 0.55))" }} />
        </div>
        <div className="relative container-page py-24 md:py-32 text-white">
          <Link to="/services" className="inline-flex items-center gap-2 text-sm text-white/80 hover:text-white"><ArrowLeft className="size-4" /> All services</Link>
      ...
      <section className="container-page py-20">
        <div className="grid md:grid-cols-3 gap-12">
                  ...
                  {sec.items.map((item, j) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div className={`size-6 rounded-full flex items-center justify-center transition-colors ${isActive ? "bg-accent text-white" : "bg-muted text-muted-foreground group-hover:bg-accent/20 group-hover:text-accent"}`}>
                            <CheckCircle2 className="size-3.5" />
                          </div>
                          <span className={`font-semibold flex-1 ${isActive ? "text-foreground" : "text-foreground/80"}`}>{item.name}</span>
                          <Plus className={`size-5 text-muted-foreground transition-all duration-300 ${isActive ? "rotate-45 text-accent" : "group-hover:text-accent"}`} />
                        </button>
          ...
          <aside className="space-y-4">
            <div className="p-6 rounded-2xl text-primary-foreground" style={{ background: "var(--gradient-hero)" }}>
              <h3 className="font-bold text-lg">Need this trade?</h3>
              <p className="mt-2 text-sm text-primary-foreground/80">Talk to our project team about scope, timeline and pricing.</p>
              <Link to="/contact" className="mt-5 inline-flex items-center gap-2 rounded-md px-4 py-2.5 text-sm font-semibold text-accent-foreground" style={{ background: "var(--gradient-gold)" }}>
                Request a Quote <ArrowRight className="size-4" />
              </Link>
            </div>
            <Link to="/services/$slug" params={{ slug: next.slug }} className="block p-6 rounded-2xl border border-border hover:border-accent transition group">
              <span className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Next service</span>
              <div className="mt-2 flex items-center justify-between">
                <span className="font-bold group-hover:text-accent transition-colors">{next.title}</span>
